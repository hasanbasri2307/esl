...
class IncomingController extends RController
{
	...
	{
		 $branch =  Yii::app()->getModule('user')->user()->profile->getAttribute('branch_id');
		 $sql = "SELECT *,sum(esc_io_detail.quantity) as total_qty_sent,sum(esc_io_detail.quantity_deliver) as total_qty_deliver FROM `esc_io` inner join esc_io_detail on esc_io_detail.io_id = esc_io.io_id inner join esc_branch on esc_branch.branch_id = esc_io.from where esc_io.to = $branch group by esc_io.io_id";
		 $connection=Yii::app()->db;
		 $command=$connection->createCommand($sql);
...
$pdf->SetTitle('Incoming Product Report');
$pdf->SetSubject('TCPDF Tutorial');
$pdf->SetKeywords('TCPDF, PDF, example, test, guide');

// set default header data

$pdf->SetHeaderData(PDF_HEADER_LOGO, PDF_HEADER_LOGO_WIDTH,'Incoming Product Report', "Euro Media");
...
$pdf->Write(0, 'Incoming Product', '', 0, 'C', true, 0, false, false, 0);
...
$pdf->Output('Laporan Incoming Product.pdf', 'I');
