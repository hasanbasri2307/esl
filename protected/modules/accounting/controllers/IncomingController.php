			...
			if($model->save()){
                              if(isset($_POST['ProductId'])){
                                    foreach ($_POST['ProductId'] as $key=>$val){
                                        ...
                                        $model_product->save();
                                       $product_stock = ProductStock::model()->find(array("condition"=>"t.product_id =:product_id AND t.branch_id=:branch_id","params"=> array(':product_id' =>$model_product->product_id,':branch_id' => $model->to,)));
                                        if($product_stock===null){
                                            $product_stock = new ProductStock();
                                            $product_stock->product_id = $val;
                                            $product_stock->branch_id = $model->to;
                                        } 
                                            $product_stock->quantity = $product_stock->quantity+$_POST["ProductQuantity"][$key];
                                            $product_stock->changed =$time =$model->changed =$time;
                                            $product_stock->save(); 
                                    }
                                }
